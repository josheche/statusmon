...
export const TOKENS_PER_XP = 10000;
...
// Generation boundaries (cumulative species ID caps)
export const GEN_CAPS = [151, 251, 386, 493, 649, 721, 809, 905, 1025];
export const SESSIONS_PER_GEN = 50;

export async function newEncounter(maxSpeciesId = 151) {
  // Retry until we get a chain whose base form is within the gen range
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    const base = stages[0];
    if (base.speciesId <= maxSpeciesId) {
      return finishEncounter(chainId, stages, base);
    }
  }
  // Fallback: Bulbasaur (chain 1)
